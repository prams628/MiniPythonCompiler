class CodeGen:
    ...
    def _regInit(self):
        ...
        self.swapRegState = {"R{}".format(i): None for i in range(self.noOfRegs, self.noOfRegs + self.swapRegCount)}

        if self.debug:
            print("The initial state of all the registers:", self.regState)
            print("The initial state of swap registers:", self.swapRegState)
    ...
    def _threeAddressListify(self):
        ...
        for code in self.threeAddressCode:
            ...
            tac.append(temp)
        if self.debug:
            self._printList(tac, "Printing the three address codes")
        return tac
    ...
    def _findAliveIntervals(self):
        ...
        if self.debug:
            self._printList(
                self.intervals, "Printing the intervals of different variables")
            ...
            if self.regState[key] and self.regState[key] not in self.aliveVars:
                ...
                if self.regState[key].startswith('"') and self.regState[key].endswith('"'):
                    self.assembly_code.append(
                        "STR {}, {}".format(chr(int(key[1]) + 64), key))
    ...
    def _allocateReg(self, var, lineno):
        ...
        self._freeReg()
        # Checking if the parameter var is a number
        if var[0] <= "9" and var[0] >= "0" or var[0] == "-":
            return "#" + var
            ...
            for reg in self.regState:
                if self.regState[reg] is None:
                    if var.startswith('"') and var.endswith('"'):
                        ...
                        self.assembly_code.insert(1, char + ": .asciz " + var)
                        self.assembly_code.append(
                            "LDR {}, ={}".format(reg, char))
        ...
        for reg in self.swapRegState:
            if self.swapRegState[reg] is None:
                self.swapRegState[reg] = var
    ...
    def _symbolTableListify(self):
        ...
        if self.debug:
            self._printList(st, "Printing the symbols")
        return st
    ...
    def generate(self):
        ...
        for lineno in range(len(self.threeAddressCode)):
            ...
            # If the current line performs a binary operation
                ...
                line += (" {}, {}, {}".format(self._allocateReg(code[0], lineno), self._allocateReg(
                    code[1], lineno), self._allocateReg(code[3], lineno)))
            # If the current line performs a relational operation
            elif code[2] in self.relOp.keys():
                self.assembly_code.append("CMP {}, {}".format(self._allocateReg(
                    code[1], lineno), self._allocateReg(code[3], lineno)))
                line = "B{} ".format(self.relOp[code[2]])
            elif code[2] == "=":
                line = "MOV "
                line += "{}, {}".format(self._allocateReg(
                    code[0], lineno), self._allocateReg(code[1], lineno))
            elif code[2] == "IfFalse":
